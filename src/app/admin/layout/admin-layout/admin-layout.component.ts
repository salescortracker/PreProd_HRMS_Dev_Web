...
export class AdminLayoutComponent {
  ...
  ngOnInit() {
    ...
    this.adminService.getUserMenus(companyId, roleId)
      .subscribe(res => {

        this.menus = res;

        this.parentMenus = this.menus.filter((x: any) => x.parentMenuId == null);

        this.childMenus = this.menus.filter((x: any) => x.parentMenuId != null);

      });

  }
  getChildMenus(parentId: number) {
  return this.childMenus.filter((x:any) => x.parentMenuId == parentId);
}
